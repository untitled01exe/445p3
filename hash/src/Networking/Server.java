...
public class Server {
    ...
    public static void main(String[] args) throws IOException, NoSuchPaddingException, NoSuchAlgorithmException {
        ...
        while(true){
            ...
            // while (true) {
                /*
                //attempt to update blockchain
                UpdateRunnable updateRunnable = new UpdateRunnable(BLOCKCHAIN);
                Thread chainUpdater = new Thread(updateRunnable);
                chainUpdater.start();

                //catch up when necessary
                new Thread(new CatchupRunnable(BLOCKCHAIN, PORT)).start();
                */
        /*
            ...
            try {
                while (true) {
                    ...
                    if(clientSocket != null) {
                        ...
                        new Thread(new ServerRunnable(clientSocket, BLOCKCHAIN, PORT)).start();
                    }
                   ...
                   // chainUpdater.start();
                    ...
                    //catch up when necessary
                    //new Thread(new CatchupRunnable(BLOCKCHAIN, PORT, userName, user)).start();
